...
/// Bot.fs's GetAutoVerdict maps to AutoVerdict.Spam when LLM_CONTENT_FILTER_IS_SPAM=true
/// (default) — see LlmTriage.fs's ClientResultException handler and Types.fs's LlmVerdict.
///
/// The scripted 400 response body below mirrors the real Azure shape (`error.code` +
/// `error.innererror.content_filter_result`), so it exercises the same raw-body-parsing path
/// production uses (`isContentFilterRejection` preferring `ex.GetRawResponse()` over a message
/// substring match).
