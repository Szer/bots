module CouponHubBot.RealTests.Program
...
/// Console helpers for the one-time MTProto setup (`make coupon-tg-login`, `make coupon-tg-chats`).
module private TgConsole =
    ...
    let private withClient (action: TgUserClient -> Task<int>) =
        task {
            ...
            if not env.CanLogin then
                eprintfn "COUPON_TG_API_ID / COUPON_TG_API_HASH / COUPON_TG_API_PHONE missing in %s" RealEnv.envFilePath
                return 1
            else
                use client =
    ...
    /// Prints every dialog with its Bot API chat id (find the CI test group id here).
    let listDialogs () =
