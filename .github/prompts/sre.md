...
Note: `verify-deploy.sh` also emits a `FAILED: 5xx error rate is non-zero` marker for a Prometheus 5xx check; the `/bot` webhook always returns HTTP 200 regardless of internal exceptions, but the marker CAN still fire from a phantom rate on frozen probe-route counters — never treat 5xx rate as proof of anything without the Step 1 raw-counter check. Do not use it as a signal; Loki `level="Error"` (above) is the real one.
...
None of the above are "is it doing its job" signals by themselves — a pod can post all-green values on every row here while the update path throws on every message (drill 3). **Never use the 5xx HTTP rate as a health metric without the Step 1 raw-counter check — it is never a real webhook signal but CAN be a phantom rate from frozen counters** (Step 1). The primary "is it actually broken" signal is the Loki query `sum(count_over_time({container="CONTAINER"} | json | level="Error"[15m]))` (Step 1a), backed by the per-bot business metrics in Step 1c.
