...
let spamDeleteReason (score: float) (actor: Actor) (reason: string option) : AutoDeleteReason =
    match actor with
    | Actor.LLM l -> AutoDeleteReason.LlmSpam {| score = score; modelName = l.modelName; reason = reason |}
    ...
    member private _.GetAutoVerdict(msg: TgMessage, usrMsgCount: int) = task {
        ...
        | Some prediction ->
            ...
            if prediction.Score > 0f && usrMsgCount >= botConfig.Value.MlOldUserMsgCount then
                ...
                return Some (AutoVerdict.Spam (float prediction.Score, Actor.ML, None))
                ...
                | LlmVerdict.Kill reason ->
                    let actor = Actor.LLM {| modelName = llmTriage.ModelName; promptHash = llmTriage.PromptHash |}
                    return Some (AutoVerdict.Spam (float prediction.Score, actor, reason))
                ...
                | LlmVerdict.Skip reason ->
                    return Some (AutoVerdict.Uncertain (float prediction.Score, reason))
                | LlmVerdict.Error | LlmVerdict.ContentFiltered _ ->
                    return Some (AutoVerdict.Uncertain (float prediction.Score, None))
            else
                return Some (AutoVerdict.NotSpam (float prediction.Score, Actor.ML))
    }
            ...
            if not shouldBeSkipped then
                ...
                | None ->
                    ...
                    | None ->
                        ...
                        | Some (AutoVerdict.Spam (score, actor, reason)) ->
                            %mlActivity.SetTag("spamScoreMl", score)
                            %mlActivity.SetTag("autoVerdict", "spam")
                            ...
                            do! enforceSpam actor (spamDeleteReason score actor reason)
