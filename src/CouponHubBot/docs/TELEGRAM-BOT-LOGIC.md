...
Бот намеренно отправляет через `TelegramNotificationService` только уведомления о том, что чужое действие изменило состояние купона у конкретного участника:
...
- **Уведомление владельцу о жалобе** (`NotifyAdderCouponReported`): когда держатель сообщает через `/report`, что купон уже использован кем-то другим, владелец получает личное сообщение.
- **Уведомление участнику при админском `/undo`** (`NotifyUndoPocketChange`): откат меняет то, что лежит у участника «в кармане», но сам по себе для него невидим — участник узнаёт об этом только от админа в личной переписке. Поэтому бот пишет ему сам:
  - откат `used` / `returned` / `reported` → купон **снова у участника**, и в сообщении сказано, что дальше он сам решает: пометить «Использован» или вернуть кнопкой «Вернуть» в `/my`;
  - откат `taken` → купон **ушёл из кармана** участника обратно в общую копилку;
  - откат `voided` → ничей карман не меняется (прежний держатель намеренно не восстанавливается), уведомление не отправляется;
  - если админ откатывает собственное действие, личного сообщения нет — он и так читает ответ на `/undo`.

  Ответ админу на `/undo` дополнительно сообщает, было ли уведомление доставлено.
...
4. **Напоминание добавить купоны** (личные сообщения):
   - Если пользователь использовал купон и с тех пор не добавил ни одного, бот на следующий день отправляет личное напоминание: «Не забудь добавить купоны в бота».
   - Сколько дней подряд повторять напоминание, задаёт `bot_setting` `ADD_COUPON_REMINDER_LOOKBACK_DAYS` (по умолчанию `2`, hot-reload через `POST /reload-settings`). При `2` напоминание приходит два дня подряд, при `1` — только один раз (прежнее поведение), при `0` или отрицательном значении не отправляется вовсе.
   - Одно напоминание в день на пользователя независимо от количества использованных купонов.
   - Проверка основывается на событиях `coupon_event`: есть `'used'` за последние `ADD_COUPON_REMINDER_LOOKBACK_DAYS` календарных дней (UTC) и нет `'added'` после последнего такого `'used'` (добавление купона сразу снимает напоминания).
