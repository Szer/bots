...
4. **Напоминание добавить купоны** (личные сообщения):
   ...
   - Напоминание повторяется **два дня подряд**: если после первого напоминания купон так и не добавлен, на следующий день приходит ещё одно. На третий день — уже нет.
   - Одно напоминание в день на пользователя независимо от количества использованных купонов.
   - Проверка основывается на событиях `coupon_event`: есть `'used'` за последние 2 календарных дня (UTC) и нет `'added'` после последнего такого `'used'` (добавление купона сразу снимает оба напоминания).
