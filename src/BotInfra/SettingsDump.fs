...
/// Backs GET /test/settings/dump (both bots) — reflects the live effective BotConfiguration
/// record, redacting any field whose name marks it secret-bearing so the endpoint can never
/// leak a token/key/connection-string value, only its presence.
module SettingsDump =
